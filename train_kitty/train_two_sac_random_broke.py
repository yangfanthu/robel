...
def eval_policy(policy, env_name, broken_info = False, eval_episodes=3, real_robot = False, seed = 0):
    ...
    for _ in range(eval_episodes):
        ...
        if broken_info:
            state = np.concatenate((state, np.ones(12)))
        while not done:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--broken-angle", type=float, default=-0.5)
    ...
    parser.add_argument('--trim-state', type=bool, default=False)
