...
if __name__ == "__main__":
    ...
    # agent.restore_model_for_test(85000)  #for pure sac
    agent.restore_model_for_test(1100000)  # for rarl sac
    # env.render()
    broken_scenarios=[]
    for i in range(9):
        for j in range(i + 1,9):
            broken_scenarios.append([i,j])
    ...
    result_dict = {}
    with torch.no_grad():
        for broken_joints in broken_scenarios:
            success_record = []
            for i in range(args.test_times):
                env.seed(args.seed + 1)
                env.action_space.seed(args.seed + 1)
                ...
                while True:
                    ...
                    if args.broken_info:
                        valve_angle = abs(current_state[-10])
                    else:
                        valve_angle = abs(current_state[-1])
                    if valve_angle < 0.3:
                        done = True
                        success = True
            ...
            success_rate = np.array(success_record).mean()
            print("success_rate", success_rate)
            result_dict[str(broken_joints)] = success_rate
    np.save('./result.npy', result_dict, allow_pickle=True)
